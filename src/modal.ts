...
const Modal = (() => {
  ...
  function _buildCarousel(photos, altText) {
    ...
    const slidesHtml = photos.map((src, i) =>
      `<div class="carousel-slide" aria-hidden="${i > 0}">
    ...
    const dotsHtml = photos.length > 1
      ? `<div class="carousel-dots">${
          photos.map((_, i) =>
            `<button class="carousel-dot${i === 0 ? ' carousel-dot--active' : ''}" data-index="${i}" aria-label="Photo ${i + 1}"></button>`
          ).join('')
        }</div>`
      : '';
  ...
  function _initCarousel(el) {
    ...
    _carouselKeyHandler = (e) => {
      if (!document.getElementById('modal-detail').classList.contains('modal--open')) return;
  ...
  function _open(modal, placeToSet) {
    closeAll();
    if (placeToSet !== undefined) currentPlace = placeToSet;

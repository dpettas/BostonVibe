...
const Render = (() => {
  ...
  function placeCard(place, userLoc) {
    ...
    if (!firstPhoto) {
      ...
      const slidesHtml = photos.map(p =>
        `<img class="card-slide" src="${escHtml(p)}" alt="${escHtml(place.name)}" onerror="this.style.visibility='hidden'">`
      ).join('');
  ...
  function mapHrefForPlace(place, coordMatch) {
    ...
    if (coordMatch) return placeSearchHref(place) || mapsSearchHref(`${coordMatch[1]},${coordMatch[2]}`);
    return raw;
  }

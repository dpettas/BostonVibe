...
const Form = (() => {
    ...
    async function _addPhotoFromFile(file) {
        ...
        if (error)
            error.textContent = 'Saving photo...';
        const upload = (async () => {
            const src = await ImageUtils.resizeAndSave(file);
            if (batchId === _photoBatchId)
                _addPhoto(src);
        })();
        ...
        try {
            ...
            if (error)
                error.textContent = 'Photo could not be saved. Make sure the local server is running.';
        }
        finally {
    ...
    async function _waitForPhotoUploads() {
        if (_pendingPhotoUploads.size === 0)
            return;
    ...
    function validate() {
        ...
        if (!mapLink)
            errors.mapLink = 'Map link or coordinates are required.';
    ...
    function showErrors(errors) {
        ...
        if (firstError && firstField[firstError])
            document.getElementById(firstField[firstError]).focus();
    }
    function _esc(str) {
